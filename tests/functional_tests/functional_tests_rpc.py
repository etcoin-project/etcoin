...
etcoind_base = [builddir + "/bin/etcoind", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--offline", "--no-igd", "--p2p-bind-port", "etcoind_p2p_port", "--rpc-bind-port", "etcoind_rpc_port", "--zmq-rpc-bind-port", "etcoind_zmq_port", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--log-level", "1"]
etcoind_extra = [
  [],
  ["--rpc-payment-address", "44SKxxLQw929wRF6BA9paQ1EWFshNnKhXM3qz6Mo3JGDE2YG3xyzVutMStEicxbQGRfrYvAAYxH6Fe8rnD56EaNwUiqhcwR", "--rpc-payment-difficulty", str(DIFFICULTY), "--rpc-payment-credits", "5000", "--data-dir", builddir + "/functional-tests-directory/etcoind1"],
]
wallet_base = [builddir + "/bin/etcoin-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--disable-rpc-login", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--daemon-port", "18180", "--log-level", "1"]
wallet_extra = [
]
...
for i in range(N_ETCOINDS):
  command_lines.append([str(18180+i) if x == "etcoind_rpc_port" else str(18280+i) if x == "etcoind_p2p_port" else str(18380+i) if x == "etcoind_zmq_port" else x for x in etcoind_base])
  if i < len(etcoind_extra):
    command_lines[-1] += etcoind_extra[i]
  outputs.append(open(builddir + '/tests/functional_tests/etcoind' + str(i) + '.log', 'a+'))
