...
Core tests take longer than any other ETcoin tests, due to the high amount of computational work involved in validating core components.
...
To run only ETcoin's core tests (after building):
...
To run only ETcoin's crypto tests (after building):
...
```bash
etcoind --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `etcoin-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only ETcoin's hash tests (after building):
...
To run only ETcoin's performance tests (after building):
...
To run only ETcoin's unit tests (after building):
